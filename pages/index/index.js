...
import {weatherBgImage} from './weatherBG'
//定义两个数组给图标传入数据
var xData = [];
var yData = [];
//给图表设置属性
...
Page({
  ...
  getCurrentCity:function(e){
    ...
    //先储存上一个城市名
    let preLocation = this.data.currentLocation
    //将存好的城市push进城市列表中
    this.data.list.push(preLocation)

    //将点击的城市的详细数据保存
    this.data.list.map(item=>{
      if(item.name === e.detail.cityName){
        this.setData({
    ...
    this.data.list.map((item,index)=>{
      if(item.name === this.data.currentLocation.name){
    ...
    wx.request({
      ...
      success(res){
        ...
        if(url =="https://devapi.qweather.com/v7/weather/now?"){
          ...
          let finishCount = 0
          weatherBgImage.map((item)=>{
            if(that.data.weatherNow.text.indexOf(item.weather) !== -1){
              query.exec((res)=>{
                that.setData({
          ...
          //没有符合的就显示默认背景图
          if(finishCount === weatherBgImage.length){
            that.setData({
              backGroundImage:weatherBgImage[weatherBgImage.length -1].bg
  ...
  getUserLocationWeather:function(){
    ...
    const that = this;
    //获取用户当前位置
    //注意：应小程序要求使用获取用户位置接口，需要在app.json中声明对应字段，并在小程序后台中开通对应的接口权限
    wx.getLocation({
      altitude: 'altitude',
      success(res){
        ...
        wx.request({
          ...
          success(res){
            ...
            wx.request({
              ...
              success(res){
                let listData = res.data.location
                listData.map((item,index)=>{
                  if(item.name === that.data.currentCity){
                    that.data.currentLocation = item
                ...
                //console.log(that.data.currentCity,that.data.list)
                //数据获取成功之后停止刷新并隐藏刷新样式
                wx.hideLoading();
